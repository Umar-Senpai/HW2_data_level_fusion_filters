...
We need to generate upsampled disparities for 12 pair of color-disparity images. For this after displaying that our filters are working, when we press any key, the code will move forward and apply the JBU, JBMU and IU algorithm on 12 sets of color-disparity images in our `data` folder. The files will then be stored inside `results` folder
...
Different parameters such as window_size, sigma etc. are tunable and can be changed for fine tuning in the main function. We also need to evaluate optimal sigmas for evaluation purposes. For this, the spatial sigma is dependent on window_size and we found that 1 window_size works the best which is equivalent to 0.4 spatial sigma. 

For spectral sigma, we have written another for loop to calculate error values on different spectral sigma values. We have defined an array of 10 spectral sigma values and we will run Iterative Upsampling algorithm on each sigma value with a fixed window_size of 7. The results generated by this piece of code are saved inside `results/sigma` directory. Its imperative to have a sigma directory already inside results.
...
### Comparison results
We have run the algorithm on 12 color-disparity pairs from Middlebury 2005 and 2006 dataset.The upsampling algorithms are run for 3 different window sizes and 3 different metrics (RMSE, PSNR, SSIM) and 3 different algorithms (JBU, JBMU, IU). The results for each metrics are shown in a separate plot.
...
#### RMSE
The plot below shows results for RMSE. Each subplot belongs to one image pair and in each subplot, we have a bar represting each algorithm. 

![Figure_1](https://user-images.githubusercontent.com/31202659/207587897-0281e17f-edf4-45ca-9c54-03191d10bda9.png)

We can see that with increasing window_size, the RMSE increases for almost all cases. Meaning that the error is increasing by increasing window_size.
...
#### Optimal Sigma
For spatial sigma, we are using window_size and the window size of 1 produces the best results. In order to figure out the optimal spectral sigma, we will run the code for different sigma values on window size of 7 and the following plots are obtained
...
Also for SSIM values, the plot for different sigmas is:
<p align="center">
